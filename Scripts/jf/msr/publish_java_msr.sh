...
if [ ${COUNT} -eq 1 ]; then
    ...
    BUILD_PORT=`echo ${MSR_INFO} | awk '{print $3}'`
    PUBLISH_SERVER=`echo ${MSR_INFO} | awk '{print $4}'`
    PUBLISH_PORT=`echo ${MSR_INFO} | awk '{print $5}'`
    echo "开始开始传送包 ${TAR_FILE}  到服务器 ${BUILD_SERVER}"
    scp -P ${BUILD_PORT} -i /root/.ssh/deploy ${TAR_FILE} ${BUILD_SERVER}:/data/maven || exit 1
    echo "开始在服务器上 ${BUILD_SERVER} 编译"
    ssh -p ${BUILD_PORT} -i /root/.ssh/deploy ${BUILD_SERVER} "sh /data/scripts/build_java_create_image.sh ${PROJECT_NAME} ${MODULE_NAME} ${PUBLISH_ENV} ${TIME_TAG}" || exit 1
    echo "开始在服务器上 ${PUBLISH_SERVER} 发布"
    ssh -p ${PUBLISH_PORT} -i /root/.ssh/deploy ${PUBLISH_SERVER} "sh /data/scripts/publish_spring_cloud.sh ${PROJECT_NAME} ${MODULE_NAME} ${PUBLISH_ENV} ${TIME_TAG}" || exit 1
else
